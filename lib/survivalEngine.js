...
export function buildOpponentTeam(pool, teamSize, playerTeam, aiLevel, playerLegendaryCount = 0) {
  const pteam = (playerTeam || []).filter(Boolean);
  const candidates = pool.filter(p => p && p.name !== "MissingNo");
  const maxBST = Math.max(1, ...candidates.map(getBST));
  // Counter-weight grows with aiLevel; noise shrinks with aiLevel
  const noise = (1 - aiLevel) * 12;

  const scored = candidates.map(p => {
    let off = 0, def = 0;
    for (const pp of pteam) {
      let bestEff = 0;
      for (const t of getTypes(p)) bestEff = Math.max(bestEff, BE.getEffectiveness(t, pp.type1, pp.type2));
      off += bestEff;
      let worstIncoming = 0;
      for (const t of getTypes(pp)) worstIncoming = Math.max(worstIncoming, BE.getEffectiveness(t, p.type1, p.type2));
  ...
  // Fill remaining slots with non-legendaries if cap blocked some picks
  ...
  return pick; // stats kept at base values
}
